...
def main(argv):
    ...
    for id_and_genre in ids_and_genres:
        ...
        feature_datas = dbControl.pullFeatureForSong("Feature_Centroid", id_and_genre[0],packsize)

        # log.debug("feature_datas.length = " + str(len(feature_datas)))
        # take 200 samples from the middle-ish
        for feature in feature_datas[2000:2200]:
            samples.append((float(feature.value),))
            classes.append(id_and_genre[1])

    machine = svm.SVC(C=2.0)
